 ...
 */
static void msh_ResizeTable(SegmentTable_t* seg_table, size_t num_segs);
 ...
 */
static size_t msh_FindNewTableSize(size_t curr_sz);
...
void msh_RemoveSegmentFromTable(SegmentTable_t* seg_table, void* key)
{
	...
	size_t removed_seg_hash;
	
	/* find the segment node immediately preceding this segment node in the table */
	removed_seg_hash = seg_table->hashfunction(seg_table, key);
	...
	else if(seg_table->compare(curr_table_node->key, key))
	{
	...
	{
		/* find the segment node immediately preceding this segment node in the table */
		while(!seg_table->compare(curr_table_node->next->key, key))
		{
...
SegmentNode_t* msh_FindSegmentNode(SegmentTable_t* seg_table, void* key)
{
	...
	if(seg_table->table != NULL)
	{
		for(curr_table_node = seg_table->table[seg_table->hashfunction(seg_table, key)]; curr_table_node != NULL; curr_table_node = curr_table_node->next)
		{
			if(seg_table->compare(curr_table_node->key, key))
			{
...
{
	SegmentTableNode_t* curr_table_node;
	size_t seg_hash = seg_table->hashfunction(seg_table, new_table_node->key);
