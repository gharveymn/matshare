function [ret] = randvargen2(v)
	[ret] = randvargen2_(1, v);
end


function [ret] = randvargen2_(currDepth, v)
	...
	switch(randvartype)
		case(1)
			...
			ret = cell(dims);
			for k = 1:numel(ret)
				ret{k} = randvargen2_(currDepth + 1, v);
			end
		case(2)
			% 	2	mxSTRUCT_CLASS
			numPossibleFields = 5;
			if(numvarsz ~= 0)
				...
				for k = 1:numel(ret)
					...
					for j = 1:numel(retFields)
						ret(k).(retFields{j}) = randvargen2_(currDepth + 1, v);
