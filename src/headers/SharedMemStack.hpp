...
public:
	...
	void deallocBuffer();                                                  /* deallocates the buffer destorying all objects pointed to */
	int reallocBuffer(size_t new_size);                                             /* reallocates the buffer to the desired size (if smaller this will fail) */
	...
	int addSegmentToBuffer(windows_shared_memory* pNewSegment,
					   mapped_region* pNewRegion);                         /* adds the segment (create by pNewSegment= new windows_shared_memory...) to the buffer.  Returns 0 if successful */
...
void shared_mem_stack::deallocBuffer()
{
	int i;
	
	/* If its allocated */
	if(pMemSegments != nullptr)
	{
...
int shared_mem_stack::reallocBuffer(size_t new_size)
{
...
int shared_mem_stack::addSegmentToBuffer(windows_shared_memory* pNewSegment, mapped_region* pNewRegion)
{
	...
	if(nSegs >= allocSegs)
	{
		if(reallocBuffer(__max((size_t)allocSegs, 0) + 50))
		{
			return -1;
		}
	}
...
int shared_mem_stack::addReferenceByName(const char* name)
{
	...
	if(index >= 0)
	{
		pMemSegments[index].refCount++;
		ret_val = 0;
	}     /*  remove the refence count */
...
{
	windows_shared_memory* pSegment = nullptr;          /*  Segment pointer to return  */
...
{
	mapped_region* pRegion = nullptr;               /*  Segment pointer to return  */
	int index;                                   /*  index of this segment */
...
int shared_mem_stack::getRefCountByIndex(int index)
{
	...
	if((index >= 0) && (index <= nSegs))
	{
		refCount = (int)pMemSegments[index].refCount;
	}
