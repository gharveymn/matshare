...
#include "headers/SharedMemory.hpp"
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	{
		mexErrMsgIdAndTxt("MATLAB:SharedMemory", "Second input argument must be a string less than %i characters", MAXDIRECTIVELEN);
	}
...
size_t deepscan(header_t* hdr, data_t* dat, const mxArray* mxInput, header_t* par_hdr)
{
	...
	if(hdr->isStruct)
	{
		...
		dat->child_hdr = (header_t*)mxCalloc(hdr->nzmax*hdr->nFields*(sizeof(header_t) + sizeof(data_t)) + strBytes, 1); /* extra space for the field string */
		...
		for(i = 0; i < hdr->nzmax; i++)                         /* each element */
		{
			...
			{
				/* call recursivley */
				hdr->shmsiz += deepscan(&(dat->child_hdr[count]), &(dat->child_dat[count]), mxGetFieldByNumber(mxInput, i, field_num), hdr);
