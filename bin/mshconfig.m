...
%
%    Note: Parameter names are case-independent. Values must all be strings.
%
%    Available configuration parameters:
%
%        ['ThreadSafety', 'ts'] -- Control whether matshare will use locks 
%                                  when dealing with shared memory.
%            Possible values: ['on','true','enable'],['off','false','disable']
%            Default: 'on'
%            Notes: I do not recommend turning this off since concurrent usage 
%                   can easily crash your session. Furthermore, matshare is 
%                   designed to be highly concurrent, so the effect of locks 
%                   is negligible in most cases.  
%
%        ['MaxVariables','mv'] -- Set the maximum number of variables 
%                                 in shared memory. 
%            Possible values: An unsigned integer less than 2^32.
%            Default: '512'
%            Notes: It is worth noting that on Linux one may hit the open 
%                   file limit set by `ulimit -n <n>`, and if this occurs 
%                   while using a parpool subprocess, there is no way of 
%                   relaying this to the main session since MATLAB uses files 
%                   to communicate. Thus, make sure this has a value lower 
%                   than the open fd limit.
%
%        ['MaxSize','ms'] -- Set the maximum total size of shared memory.
%            Possible values: An unsigned integer less than the maximum size 
%                             on your system.
...
%                   use `mshpersistshare` to share the variable, which will 
%                   disable garbage collection of that variable.
%      
%        ['Security','sc'] -- Set the security of shared memory segments.
%            Possible values: An octal security value
%            Default: '0600'
